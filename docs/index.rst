...
``mapc-sim`` is a simulation tool for IEEE 802.11 Multi-Access Point
...
-  **Simulation of C-SR**: You can simulate the C-SR performance of an
   ...
   (MCS). Calculate the aggregated effective data rate.
-  **TGax channel model**: The simulator incorporates the TGax channel
...
The repository `(ml4wifi-devs/mapc-sim) <https://github.com/ml4wifi-devs/mapc-sim>`_ is structured as follows:

-  ``mapc_sim/``: Main package containing the simulator.

   -  ``constants.py``: Physical and MAC layer constants used in the
      simulator.
...
The main functionality is provided by the ``network_data_rate`` function
in ``mapc_sim/sim.py``. This function calculates the effective data rate
for a given network configuration. Example usage:
...
Earlier versions of the simulator took a binary ``walls`` matrix and scaled it
internally by a fixed per-wall loss. That call style still works: pass ``walls``
(and optionally ``wall_loss``) **by keyword** and the
:func:`mapc_sim.utils.accepts_walls` decorator converts it to a ``loss_gain``
matrix, with a ``DeprecationWarning``.
...
A matrix passed positionally is always the ``loss_gain`` matrix -- the two cannot
be told apart, since under ``jit`` the values are tracers and a binary matrix is a
valid ``loss_gain`` matrix in its own right.
...
The simulator is written in JAX, which enables just-in-time (JIT) compilation
...
apply the `jax.jit` transformation on the simulator function:
...
The simulator uses JAX's PRNG. This ensures that the simulator is fully reproducible.
...
two keys in each step of a simulation:
...
If you want to use 64-bit floating point precision, you can set the appropriate
...
-  The simulator is written in JAX, an autodiff library for Python. It
   ...
   JAX, please refer to the official `JAX
